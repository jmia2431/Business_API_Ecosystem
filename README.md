...
部署完成后，任何访客都可以提交 API。成功提交会写入 Supabase，状态为 `Draft`；刷新页面后数据仍在数据库中，但不会在公共目录显示，直到你审核通过。静态 56 条数据与已审核的数据库记录会按“Company + API name”去重合并，并且访客不能用同名提交覆盖基础数据。

审核方法：进入 Supabase **Table Editor → apis**，检查来源和文档后，把 `review_status` 从 `Draft` 改为 `Verified candidate` 或 `Published`。刷新网站后该记录就会公开显示。
